            ...
            <div class="card-body mt-3">
                <div class="text-center mb-5">
                    @if (Auth::user()->image == 'default/user.jpg')
                        <img src="{{ asset('default/user.jpg') }}" alt="image" class="img-thumbnail img-preview" width="100px">
                    @else
                        <img src="{{ asset('storage/users/' . Auth::user()->image) }}" alt="image" class="img-thumbnail img-preview" width="100px">
                    @endif
                    
                    <div class="text-center">
                        <form action="{{ route('profil.destroy', $profile) }}" method="POST">
                            @csrf
                            @method('DELETE')
                ...
                <form action="{{ route('profil.store') }}" class="row gy-3 needs-validation" method="post" enctype="multipart/form-data" novalidate>
                    @csrf
                    <div class="col-sm-12">
                    ...
                    <div class="col-sm-12">
                        <div class="form-group">
                            ...
                            <input id="phone" type="number" class="form-control @error('phone') is-invalid @enderror" name="phone" value="{{ $profile->phone ?? '' }}" placeholder="Telepon">
                            @error('phone')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
